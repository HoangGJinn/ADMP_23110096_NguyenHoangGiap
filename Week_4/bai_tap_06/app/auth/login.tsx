...
export default function LoginScreen() {
  ...
  // Watch for authentication changes and navigate
  useEffect(() => {
    if (isAuthenticated) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // useEffect will handle navigation when isAuthenticated changes
    } catch (error: any) {
      ...
      if (errorMessage.includes('chưa được xác thực') || 
        ...
        // Pattern: "email@example.com" or check if userName is an email
        let emailToPass = '';
        const emailMatch = errorMessage.match(/[\w.-]+@[\w.-]+\.\w+/);
        if (emailMatch) {
          emailToPass = emailMatch[0];
        } else if (userName.includes('@')) {
          // User might have logged in with email
          emailToPass = userName;
        }
          ...
          [
            ...
            { 
              ...
              onPress: () => router.push({
                ...
                params: emailToPass ? { email: emailToPass } : {}
              })
            }
          ]
        );
